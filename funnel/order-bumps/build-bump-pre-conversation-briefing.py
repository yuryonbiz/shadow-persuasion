...
def build_docx():
    ...
    selector = [
        ("Salary / raise conversation",
         "Low-Status Opening (book p.41), Strategic Pause (p.78), The One-Line Email Lock-In (p.129)"),
        ("Hard conversation with partner",
         "Cold Open (p.34), The Specific Memory Citation (p.92), Validating Without Agreeing (p.101)"),
        ("Reconnection with estranged family",
         "Unrelated Reference Open (p.37), Micro-Disclosure (p.48), Permission Frame (p.56)"),
        ("Custody / divorce mediation",
         "Accusation Audit (p.46), Boundary Without Anger (p.118), Document the Concern (p.125)"),
        ("Hostile customer / client",
         "The Accusation Audit (p.46), Labeling What They Feel (p.88), Thank-Then-Clarify (p.127)"),
        ("Contractor / service price dispute",
         "The 11-Second Silence (p.79), \u201cWhat would make you comfortable?\u201d (p.124), Stake-In-Ground close (p.132)"),
        ("Parent / teenager communication",
         "Permission Frame (p.56), Planting the Conclusion (p.66), The Observer Frame (p.108)"),
        ("Business / sales discovery call",
         "Reverse Qualifier Opener (p.52), Socratic Cascade (p.63), Calendar Pull close (p.131)"),
        ("Confronting a manipulator",
         "DARVO Detection (p.114), Grey Rock Response (p.117), The Witness Frame (p.121)"),
        ("Reconciling after a fight",
         "Micro-Disclosure (p.48), Parallel Vulnerability (p.87), The 24-Hour Lock email (p.137)"),
        ("Job interview / being interviewed",
         "Low-Status Opening (p.41), The Question That Flatters (p.94), Research Credibility Drop (p.50)"),
        ("Any conversation you're avoiding",
         "Pre-Frame Drop (p.38), The Pause Before the Ask (p.55), Commitment Without Ask (p.133)"),
    ]
